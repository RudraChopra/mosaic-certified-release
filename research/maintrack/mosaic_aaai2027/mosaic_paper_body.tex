...
For a finite transform polytope, the resulting problem is solved globally by
decoder enumeration and linear optimization. In a hash-locked synthetic confirmation,
naive continuum selection deploys contract-violating releases on 42.1\% of
1,000 primary tables, while MOSAIC records none; in a feasible regime MOSAIC
safely deploys on 56.3\% of tables versus 3.3\% for finite Learn-Then-Test.
Its deployment curve tracks a prediction frozen before outcomes with 0.20
percentage-point mean absolute error. An independent program replays every
channel, exact risk, and decision from 64,000 method-level receipts.
\end{abstract}
...
dearteaga2019bios}. For every dataset and each untouched seed in
