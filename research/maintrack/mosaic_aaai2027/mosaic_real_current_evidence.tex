...
\paragraph{High-support $K=8$ extension.}
A separate post-review protocol fixes three employment transfers before
execution and raises both certification tables from 6,000 to 16,000 balanced
rows per source--label stratum (64,000 reference and 64,000 bridge rows per
job). It evaluates the complete 13-candidate official frontier in every job,
for 39 rows total. At the same $.40$ primary utility threshold, strict MOSAIC
certifies a nonconstant release on Florida and New York; Illinois remains an
abstention. The two released interfaces have safe untouched diagnostics and no
contract violation in either 100-draw operational replay.
...
\begin{tabular}{lccc}
\hline
Target & Strict selection & Diagnostic error & Operational violations \\
\hline
Florida & INLP, rank 8 & .2978 & 0/100 \\
Illinois & abstain & -- & -- \\
New York & MANCE++ & .2772 & 0/100 \\
\hline
\end{tabular}
\caption{Locked high-support $K=8$ employment extension at
$\tau_U=.40$. The certified error upper bounds are $.3849$ (Florida) and
$.3716$ (New York), both below the contract.}
\label{supp:tab:k8-high-support}
\end{table}

This is a measured support effect, not a broad claim that $K=8$ is solved:
the protocol covers three employment transfers at one locked seed, and one of
them still abstains. The frozen audit checks all 39 candidate rows, every
official-method provenance record, both sampling caps, and the 200 operational
draws.
...
\paragraph{Complementary direct-region scope stress.}
The preceding construction is intentionally favorable to the bridge: it moves
to a law inside the learned bridge class and outside the direct target region.
We therefore also audit the opposite geometry using the two invalid target laws
from the prelocked 12,000-table bridge-misspecification study. Their population
retained masses are $.775$ and $.50$, below the declared $.80$ minimum. Among
the 8,000 invalid rows, the known target law remains inside every rowwise direct
target region in 7,983 cases (3,987 underdeclared-contamination and 3,996
source-specific-transform cases). MOSAIC's membership gate abstains on all
8,000 rows, including all 7,983 direct-region cases.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3.2pt}
\begin{tabular}{p{.42\columnwidth}p{.21\columnwidth}p{.21\columnwidth}}
\hline
Target-law scope & Direct-region rows & MOSAIC abstains \\
\hline
Bridge-admitted, outside direct region & 0/36 & 16/36 \\
Direct-region, bridge-invalid & 7,983/8,000 & 8,000/8,000 \\
\hline
\end{tabular}
\caption{Complementary scope cells. The first is the post-hoc real-table
stress; the second is a deterministic recount of prelocked bridge-invalid
tables.}
\label{supp:tab:symmetric-scope}
\end{table}

This is a scope check, not a claim that the direct target-table rule fails
inside its own confidence region: its sampling statement remains intact there.
The comparison instead makes the two declared sets visible and shows the
bridge's operational tripwire when its structural assumption is violated. The
frozen receipt records every rowwise membership calculation.
