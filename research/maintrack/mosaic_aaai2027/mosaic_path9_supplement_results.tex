...
\section{Running-Case Certificate Decomposition}

For the BiasBios-Clinical seed-1200 interface used in the main paper, the exact
utility envelope in each source--label stratum decomposes as
\[
(1-\eta)\widehat p^\top T u
\;+\;(1-\eta)\{\Phi(\widehat p,\epsilon;Tu)-\widehat p^\top Tu\}
\;+\;\eta\max_c u(c).
\]
The three terms are respectively the fitted common-law error, finite-sample
support charge, and arbitrary-residual charge. Table~\ref{supp:tab:path9-decomposition}
replays the stored counts, radii, transforms, channel, and decoder.
...
\hline
$y$ & $s$ & Common law & Finite sample & Residual & Total \\
\hline
0 & 0 & .05921484 & $9.23\!\times\!10^{-9}$ & .20107954 & .26029439 \\
0 & 1 & .05921484 & $9.23\!\times\!10^{-9}$ & .20107954 & .26029439 \\
1 & 0 & .05640794 & $2.00\!\times\!10^{-8}$ & .20388639 & .26029436 \\
1 & 1 & .05640797 & $2.00\!\times\!10^{-8}$ & .20388639 & .26029439 \\
\hline
\end{tabular}
\caption{Exact decomposition of the running-case worst-stratum error
certificate. The bridge residual, not the multinomial support charge, dominates
the gap for this interface.}
\label{supp:tab:path9-decomposition}
\end{table}

\section{Large Released-Alphabet Constraint Generation}

A review-triggered study was locked before outcomes at commit
\texttt{ed7d8b7}. It fixes $K=16$, two labels, three seeds, 2,000 observations
per source--label stratum, identity common transforms, contamination $.05$,
source contract $.35$, and utility contract $.40$. For each released alphabet,
one balanced binary decoder is fixed and the optimizer adds only attacker
assignments that violate the current LP solution. Every returned channel is
then checked against the complete attacker family.
...
\begin{tabular}{ccrrrr}
\hline
$G$ & $L$ & Solved & Retained & Median sec. & Active / full \\
\hline
4 & 2  & 3/3 & 3/3 & .027 & 4/16 \\
4 & 4  & 3/3 & 3/3 & .111 & 4/256 \\
4 & 8  & 3/3 & 3/3 & 22.302 & 4/65,536 \\
2 & 2  & 3/3 & 3/3 & .008 & 2/4 \\
2 & 4  & 3/3 & 3/3 & .011 & 2/16 \\
2 & 8  & 3/3 & 3/3 & .055 & 2/256 \\
2 & 16 & 3/3 & 3/3 & 11.446 & 2/65,536 \\
\hline
\end{tabular}
\caption{Locked synthetic released-alphabet scaling. ``Active'' is the maximum
number of attacker assignments inserted into any LP in the cell; exact
post-solve evaluation ranges over the full family.}
\label{supp:tab:path9-large-release}
\end{table}

Median certified worst-stratum error is $.1165$ for $G=4$ and $.1143$ for
$G=2$ at every tested $L$. A larger released alphabet cannot worsen this fixed
decoder construction because the optimizer may assign zero mass to extra
outputs. The result establishes feasibility of exact constraint generation at
$L=8$ and $L=16$ for these synthetic laws. It does not claim that natural
large-output interfaces retain at the same rate.
