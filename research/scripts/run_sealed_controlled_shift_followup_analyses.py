"""Generate and seal the controlled-shift follow-up analysis artifacts."""
...
def require_committed_analysis_sources(repo: Path) -> None:
    result = subprocess.run(
        [
            ...
            "research/scripts",
            "research/tests",
            "research/prereg_controlled_shift_followup.json",
            "research/prereg_controlled_shift_followup.sha256",
        ],
        cwd=repo,
        check=True,
        capture_output=True,
...
def main() -> int:
    ...
    paths = {
        "independent_followup_replay": output_dir / "independent_followup_replay.json",
        "protocol_followup_cap8_analyzer": output_dir
        / "protocol_followup_cap8_analyzer.json",
    }
    ...
    commands = {
        "independent_followup_replay": [
            str(args.python),
            str(scripts / "independent_replay_followup.py"),
            *shared,
            "--output",
            str(paths["independent_followup_replay"]),
        ],
        "protocol_followup_cap8_analyzer": [
            str(args.python),
            str(scripts / "analyze_controlled_shift_followup_cap8.py"),
            *shared,
            "--output",
            str(paths["protocol_followup_cap8_analyzer"]),
        ],
    }
    execution: dict[str, dict[str, object]] = {}
    artifacts: dict[str, dict[str, object]] = {}
    for name in ("independent_followup_replay", "protocol_followup_cap8_analyzer"):
    ...
    manifest = {
        "schema_version": 1,
        "name": "VERA sealed controlled-shift follow-up analysis manifest",
