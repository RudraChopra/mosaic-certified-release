"""Independent finite-reference replay for the VERA controlled-shift follow-up.
...
def run_replay(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if observed_receipts != expected_receipts or len(observed_receipts) != expected_count:
        raise RuntimeError(
            "receipt file set differs from the exact "
            f"{expected_count:,}-run matrix"
        )
    ...
    expected_archives = len(SEEDS) * len(DATASETS) * 12
    if len(archive_paths) != expected_archives:
    ...
    return {
        "schema_version": 1,
        "name": "VERA independent finite-reference follow-up replay",
