#!/usr/bin/env python3
"""Freeze MOSAIC's real-feature protocol before confirmation outcomes."""
...
DEFAULT_TEMPLATE = ROOT / "prereg_mosaic_real_v1.template.json"
DEFAULT_OUTPUT = ROOT / "prereg_mosaic_real_v1.json"
CODE_PATHS = (
    "research/mosaic/audit_mosaic_real_frontier.py",
    "research/mosaic/lock_mosaic_real_prereg.py",
    ...
    "research/mosaic/run_mosaic_official_frontier_pilot.py",
    "research/mosaic/run_mosaic_real_confirmation.py",
...
def validate_template(template: dict[str, object]) -> None:
    ...
    if seeds != [100, 101, 102, 103, 104] or excluded.intersection(seeds):
...
def pilot_hashes() -> dict[str, str]:
    artifacts = REPOSITORY / "research" / "artifacts"
    paths = sorted(artifacts.glob("mosaic_real_pilot*.json"))
    paths.extend(sorted(artifacts.glob("mosaic_official_frontier_pilot*.json")))
    unique = sorted(set(paths))
    if not unique:
        raise ValueError("pilot history is missing")
    return {
...
def main() -> None:
    ...
    confirmation_dir = REPOSITORY / "research" / "artifacts" / "mosaic_real_confirmation_v1"
    ...
    locked.update(
        {
            ...
            "pilot_artifact_sha256": pilot_hashes(),
            "frozen_stores": {
                name: {
                    "path": str(path),
