#!/usr/bin/env python3
"""Execute the locked, fully preflighted high-support K=8 ACS extension."""
...
from lock_mosaic_acs_k8_high_support_v3 import (
    JOBS,
    OUTPUT,
    RECEIPT_ROOT,
    STORE_ROOT,
...
def main() -> None:
    ...
    if args.receipt_root.exists() or args.summary.exists():
        raise FileExistsError("K=8 high-support v3 outcome path already exists")
    sidecar = args.lock.with_suffix(args.lock.suffix + ".sha256")
    if sidecar.read_text(encoding="utf-8").strip() != sha256(args.lock):
        raise AssertionError("K=8 high-support v3 lock sidecar mismatch")
    lock = json.loads(args.lock.read_text(encoding="utf-8"))
    if runtime_preflight() != lock["runtime_preflight"]:
        raise AssertionError("locked official-adapter runtime changed")
    ...
    for job in lock["jobs"]:
        ...
        payload = runner.run_job(
            task=str(job["task"]), target_state=str(job["target_state"]),
            seed=int(job["seed"]), store_path=store,
        )
        if (
            payload["sample_counts"]["reference"] != int(lock["maximum_reference_rows"])
            or payload["sample_counts"]["bridge"] != int(lock["maximum_bridge_rows"])
        ):
        ...
        payload["analysis_status"] = "post-review, pre-outcome K=8 high-support extension v3"
    ...
    summary = {
        "name": "MOSAIC K=8 high-support ACS extension v3",
        "status": "complete",
