#!/usr/bin/env python3
"""Replay every diagnostic-anchored released-interface utility calculation."""
...
from run_mosaic_release_utility_v2 import validate_lock
...
def main() -> None:
    ...
    stored = {
        (row["dataset"], int(row["seed"]), row["utility_threshold"]): row
    ...
    for row in recomputed:
        ...
        key = (row["dataset"], int(row["seed"]), row["utility_threshold"])
        if stored.get(key) != row:
            failures.append(str(key))
    payload = {
        "name": "MOSAIC diagnostic-anchored utility independent replay",
    ...
    if failures:
        raise AssertionError("utility repair replay found mismatches")
