#!/usr/bin/env python3
"""Compose pinned official erasers with MOSAIC in exploratory real-data pilots."""
...
def select_certified_result(results: list[dict[str, object]]) -> dict[str, object]:
    """Select the lowest-error certified candidate, or explicitly abstain."""
    eligible = [
        result
        for result in results
        if result.get("deployed") is True
        and isinstance(result.get("certified_worst_conditional_error"), (int, float))
    ]
    if not eligible:
        return {
            "decision": "abstain",
    ...
    selected = min(
        eligible,
        key=lambda result: (
            float(result["certified_worst_conditional_error"]),
    ...
    return {
        ...
        "certified_worst_conditional_error": selected[
            "certified_worst_conditional_error"
        ],
        "certified_privacy_advantages": selected["certified_privacy_advantages"],
        "external_estimable": selected["external_estimable"],
        "external_worst_privacy_advantage": selected[
            "external_worst_privacy_advantage"
        ],
        "external_worst_conditional_error": selected[
            "external_worst_conditional_error"
        ],
        "external_safe": selected["external_safe"],
        "false_acceptance": selected["false_acceptance"],
    }
...
def candidate_result(
    ...
    *,
    ...
    deploy = solution.certified_worst_conditional_error <= utility_threshold + 1e-10
    external_safe = bool(
    ...
    return {
        ...
        "false_acceptance": bool(deploy and external_risk.estimable and not external_safe),
...
def main() -> None:
    ...
    output = args.output or (
        ...
        / "research"
        / "artifacts"
        / f"mosaic_official_frontier_pilot_{args.dataset.lower().replace('-', '_')}_seed{args.seed}.json"
    )
    payload = {
        "name": (
            "MOSAIC official-erasure real-feature confirmation"
            if prereg is not None
            else "MOSAIC official-erasure frontier exploratory pilot"
        ),
        "status": (
        ...
        "selection": select_certified_result(results),
        "results": results,
    }
