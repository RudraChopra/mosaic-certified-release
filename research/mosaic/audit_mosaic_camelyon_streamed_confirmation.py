#!/usr/bin/env python3
"""Replay and audit the locked streamed Camelyon17 confirmation."""
...
RUNNER = (
    ROOT
    / "research/mosaic/"
    "run_mosaic_camelyon_streamed_confirmation.py"
)
...
def validate_summary(
    ...
    expected = {
        ...
        "main_paper_inclusion_gate_passed": gate,
    }
    for key, value in expected.items():
        if summary[key] != value:
            raise RuntimeError(f"Camelyon summary field differs: {key}")
    expected_receipts = [
        {
            "path": f"seed-{seed}.json",
            "sha256": sha256(result_dir / f"seed-{seed}.json"),
            "status": receipt["status"],
        }
        for seed, receipt in zip(seeds, receipts, strict=True)
    ]
    if summary["receipts"] != expected_receipts:
        raise RuntimeError("Camelyon receipt hash manifest differs")
    return {
...
def main() -> None:
    ...
    try:
        if sidecar.read_text(encoding="utf-8").split()[0] != lock_digest:
            raise RuntimeError("Camelyon lock sidecar mismatch")
        manifest = load(args.store / "manifest.json")
        if manifest["preregistration_sha256"] != lock_digest:
            raise RuntimeError("Camelyon store belongs to another lock")
        if (
            manifest["metadata_sha256"]
            != prereg["metadata"]["sha256"]
        ):
            raise RuntimeError("Camelyon metadata hash differs")
        if int(manifest["n_examples"]) != int(
            prereg["streamed_store"]["selected_rows"]
        ):
            raise RuntimeError("Camelyon row count differs")
        if int(manifest["dimension"]) != 512:
            raise RuntimeError("Camelyon feature dimension differs")
        arrays = {
        ...
        if (
            ...
            raise RuntimeError("Camelyon selected IDs differ")
        checks["store"] = {
        ...
        with tempfile.TemporaryDirectory(
            ...
            names = [
                *(f"seed-{seed}.json" for seed in prereg["seeds"]),
                "summary.json",
            ]
            for name in names:
                if normalize(load(args.results / name)) != normalize(
                    load(replay / name)
                ):
                    raise RuntimeError(
                        f"Camelyon replay differs for {name}"
                    )
    ...
    report = {
        "name": "MOSAIC streamed Camelyon17 confirmation audit v1",
        "pass": not failures,
