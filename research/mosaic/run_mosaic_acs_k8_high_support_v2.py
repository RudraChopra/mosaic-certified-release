#!/usr/bin/env python3
"""Execute the locked, dependency-checked high-support K=8 ACS extension."""
...
from lock_mosaic_acs_k8_high_support_v2 import (
    JOBS,
    OUTPUT,
    RECEIPT_ROOT,
    STORE_ROOT,
...
def main() -> None:
    ...
    if args.receipt_root.exists() or args.summary.exists():
        raise FileExistsError("K=8 high-support v2 outcome path already exists")
    try:
        import tqdm  # noqa: F401
    except ModuleNotFoundError as exc:
        raise RuntimeError("tqdm disappeared after the protocol was locked") from exc
    sidecar = args.lock.with_suffix(args.lock.suffix + ".sha256")
    if sidecar.read_text(encoding="utf-8").strip() != sha256(args.lock):
        raise AssertionError("K=8 high-support v2 lock sidecar mismatch")
    ...
    for job in lock["jobs"]:
        ...
        payload["analysis_status"] = "post-review, pre-outcome K=8 high-support extension v2"
    ...
    summary = {
        "name": "MOSAIC K=8 high-support ACS extension v2",
        "status": "complete",
